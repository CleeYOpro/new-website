...
async function simulateReply(text: string): Promise<string> {
  ...
  if (q.includes('hire') || q.includes('available') || q.includes('job') || q.includes('work'))
    return "cleo is a high school junior actively building and open to interesting opportunities. best way to reach him is cbalaranjith@gmail.com.";
  if (q.includes('tech') || q.includes('stack') || q.includes('language') || q.includes('tool'))
    return "he works with VS Code, GitHub, Supabase, React, TypeScript, Python, and Vercel. his fav stack rn.";
  ...
  if (q.includes('experience') || q.includes('work history') || q.includes('internship'))
    return "he's interned at Rove (YC W24) on airport search autocomplete, did business strategy at Seattle Sports & Regenerative Medicine, and observed ops at CMC Vellore — top 5 hospital in asia.";
  if (q.includes('school') || q.includes('education') || q.includes('college'))
    return "he's doing Running Start at Bellevue College while attending Eastlake High School. also got into MIT Beaver Works Summer Institute (~6% acceptance) for remote sensing.";
  ...
  return "great question. cleo is a high school junior building practical stuff across software, GIS, and engineering. for specifics, reach him directly at cbalaranjith@gmail.com.";
}
